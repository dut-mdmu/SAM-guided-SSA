...
class SkillVideoDataset(Dataset):
    ...
    def __read_frame_tensors (self, video_name, frame_idx_list, task_name):
        frame_images_dir = os.path.join(self.root_dir, task_name, f'{task_name}_160x120-seg',
        ...
        frame_tensors = torch.stack(frame_tensors, dim=0).mean(dim=1, keepdim=True)
        mask = (frame_tensors > 0).float()

        frame_images_dir1 = os.path.join(self.root_dir, task_name, f'{task_name}_160x120',
                                         video_name.split('_')[-1], 'frame')
        frame_tensors1 = [self.transform(Image.open(os.path.join(frame_images_dir1,
                                                                 f'{fidx:05d}.jpg'))) for fidx in frame_idx_list]
        frame_tensors1 = torch.stack(frame_tensors1, dim=0).mean(dim=1, keepdim=True)
        frame_tensors = torch.cat([frame_tensors, frame_tensors1, mask], dim=1)
