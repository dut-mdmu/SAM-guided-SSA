...
def train(args, split_index, device, save_label):
    ...
    train_save_path = os.path.join(save_dir, f"{save_label}_split{split_index}_results_train.xlsx")
    val_save_path = os.path.join(save_dir, f"{save_label}_split{split_index}_results_val.xlsx")
    ...
    train_result = {}
    val_result = {}
    all_result = {'train': train_result, 'val': val_result}
    since = time.time()
    save_loss = 100
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # save_path[phase].write("Video_Name\t")
            for i, video_name in enumerate(all_video_names):
                if video_name not in all_result[phase].keys():
                    all_result[phase][video_name] = []
                all_result[phase][video_name].append([pred_scores[i], gt_scores[i]])
            out_csv = []
            print(all_result)
            print(all_video_names)
            temp = ['Video_Name']
            temp.extend(list(all_result[phase].keys()))
            out_csv.append(temp)
            for i in range(epoch + 1):
                temp = [f'Result(Epoch:{i + 1})']
                for key in list(all_result[phase].keys()):
                    temp.append(str(all_result[phase][key][i][0]) + '-' + str(all_result[phase][key][i][1]))
                out_csv.append(temp)
            # print(out_csv)
            out_csv = np.array(out_csv)
            out_csv = pd.DataFrame(out_csv)
            out_csv.to_excel(save_path[phase], index=False, header=False)

            print(f'{phase} Loss: {epoch_loss:.4f} Rho: {epoch_rho:.4f} P-value: {epoch_pvalue:.4f}.')
            if phase == 'val':
                if args.save_checkpoint:
                    if save_loss > epoch_loss:
