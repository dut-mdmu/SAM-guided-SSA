...
class MaskedFeatureExtractor(nn.Module):
    def __init__(self, in_channels=3, feat_channels=256):
        ...
        self.channel_attn = nn.Sequential(
            ...
            nn.Conv2d(feat_channels, feat_channels // 16, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(feat_channels // 16, feat_channels, kernel_size=1),
            nn.Sigmoid()
        )

        self.spatial_attn = nn.Sequential(
            nn.Conv2d(feat_channels, 1, kernel_size=7, padding=3),
            nn.Sigmoid()
        )

        self.masked_norm = MaskedBatchNorm2d(feat_channels)

    def generate_mask(self, x):
        # 同时考虑像素值为0的区域和学习到的语义掩码
        pixel_mask = (x > 1e-5).float()
        semantic_mask = self.mask_generator(x)
        pixel_mask = F.interpolate(pixel_mask, size=semantic_mask.shape[2:])
        combined_mask = pixel_mask * semantic_mask

        return combined_mask.transpose(1, 2).contiguous()

    def forward(self, x, features):
        mask = self.generate_mask(x)
        b, l, c, h, w = mask.shape
        mask = mask.view(b * l, -1, h, w)
        mask = F.interpolate(mask, size=features.shape[2:],
                             mode='bilinear', align_corners=False).mean(dim=1, keepdim=True)
        norm_features = self.masked_norm(features, mask)

        channel_weights = self.channel_attn(norm_features)
        channel_enhanced = norm_features * channel_weights

        spatial_weights = self.spatial_attn(channel_enhanced)
        spatial_enhanced = channel_enhanced * spatial_weights

        fused_features = self.feature_fusion(torch.cat([features, spatial_enhanced], dim=1))

        masked_features = fused_features * mask
...
class MaskedBatchNorm2d(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        count = torch.max(count, torch.tensor(1.0, device=x.device))

        mean = masked_x.sum((2, 3), keepdim=True) / count
        var = ((masked_x - mean) * mask).pow(2).sum((2, 3), keepdim=True) / count
...
class PartGroup_SkillNet3D (nn.Module):
    def __init__ (self, num_parts=3, extractor_type='r3d', context_type='none', aggregate_type='mean',
        ...
        if extractor_type == 'r2p1d':
            ...
            self.featmap_extractor = nn.Sequential(*list(self.extractor.children())[:-2])  # 512, 7x7
            self.num_chnals = self.featmap_extractor[-1][-1].conv2[1].num_features  # 512
        # self.num_chnals = self.num_chnals * 2
        if self.num_parts > 1:
        ...
        if context_type == 'lstm':
            ...
            self.context_out_size = self.part_out_chnals
        # self.masked_feat_extractor = MaskedFeatureExtractor(in_channels=3, feat_channels=256)
        if self.scene_node:
    ...
    def forward (self, clip_tensor):
        ...
        if len(clip_tensor.shape) == 5:
            ...
            # bs, l, c, h, w = clip_tensor.shape
            # clip_tensor = clip_tensor.transpose(1,2).contiguous()
            # featmaps_masked, masks = self.masked_feat_extractor(clip_tensor, featmaps)
            # featmaps = featmaps * masks
            # featmaps_masked, masks = self.masked_feat_extractor(clip_tensor, featmaps)
            # featmaps = torch.cat([featmaps, featmaps_masked], dim=1)
