...
##Knot_Tying
 # python main_jigsaws_group.py --split_index 0 --num_samples 32 --batch_size 4 --val_split SuperTrialOut --task Knot_Tying --num_epochs 40 --schedule_step 20 --learning_rate 3e-5 --num_parts 3 --shaping_weight 10 --scene_node --visualize
...
def batch_process_videos(input_dir, output_root_dir, fps, width):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.avi'):
            ...
            # 使用正则表达式提取视频编号和capture编号
            match = re.search(r'Knot_Tying_([A-Z]\d+)_capture(\d+)', filename)
            if match:
                video_number = match.group(1)
                capture_number = int(match.group(2))
                videos.append((video_path, video_number, capture_number))
...
# 定义输入文件夹和输出根目录
input_dir = '/root/autodl-tmp/Surgical-Skill-Assessment-via-Video-Semantic-Aggregation/data/jigsaws/jigsaws_annot/Knot_Tying/video'
output_root_dir = '/root/autodl-tmp/Surgical-Skill-Assessment-via-Video-Semantic-Aggregation/data/jigsaws/Knot_Tying/Knot_Tying_160x120'
...
##Needle_Passing
 # python main_jigsaws_group.py --split_index 1 --num_samples 32 --batch_size 4 --val_split SuperTrialOut --task Needle_Passing --num_epochs 40 --schedule_step 20 --learning_rate 3e-5 --num_parts 3 --shaping_weight 10 --scene_node --visualize
...
#             # 使用正则表达式提取视频编号和capture编号
#             match = re.search(r'Needle_Passing_([A-Z]\d+)_capture(\d+)', filename)
#             if match:
#                 video_number = match.group(1)
#                 capture_number = int(match.group(2))
#                 videos.append((video_path, video_number, capture_number))
...
# # 定义输入文件夹和输出根目录
# input_dir = '/root/autodl-tmp/Surgical-Skill-Assessment-via-Video-Semantic-Aggregation/data/jigsaws/jigsaws_annot/Needle_Passing/video'
# output_root_dir = '/root/autodl-tmp/Surgical-Skill-Assessment-via-Video-Semantic-Aggregation/data/jigsaws/Needle_Passing/Needle_Passing_160x120'
